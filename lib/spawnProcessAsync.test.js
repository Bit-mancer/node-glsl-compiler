...
describe( 'spawnProcessAsync', () => {
    ...
    function emitErrorEvent( error ) {
        process.nextTick( () => spawnMock.on.mock.calls[0][1]( error ) );
    }
    ...
    it( 'rejects the Promise with a ProcessError if the child process fails to spawn', cb => {
        ...
        .catch( err => {
            ...
            expect( err.message ).toEqual( jasmine.stringMatching( /spawn error/ ) );
    ...
    it( 'rejects the Promise with a ProcessError if the child process exits with a non-zero exit code', cb => {
    ...
    it( 'rejects the Promise with a ProcessError if the child process is terminated by a signal', cb => {
        ...
        .catch( err => {
            expect( err ).toEqual( jasmine.any( ProcessError ) );
            expect( err.code ).toEqual( 1 );
            expect( err.signal ).toEqual( 'SOMESIG' );
            cb();
        });
